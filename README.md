...
```powershell
Invoke-WebRequest -Uri "https://github.com/lzdrahal/pc-maintenance/archive/refs/heads/master.zip" -OutFile "$env:TEMP\maintenance.zip"; Expand-Archive "$env:TEMP\maintenance.zip" -DestinationPath "$env:TEMP\maintenance" -Force; Set-Location "$env:TEMP\maintenance\pc-maintenance-master"; powershell.exe -ExecutionPolicy Bypass -File .\Run-Maintenance.ps1
```
